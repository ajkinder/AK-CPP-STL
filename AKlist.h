...
//  AKlist.h
//
//  Created by AlexKinder on 4/10/20.
//
...
public:
   ...
   T& back() { return mTail->prev->data; }
   const T& back() const { return mTail->prev->data; }
   ...
   protected:
      ...
      Node *mCurrent;
      const AKlist *mMyList;  // needed to test for certain errors

      // protected constructor for use only by derived iterator and friends
      const_iterator(Node *p, const AKlist<T> &lst) : mCurrent(p), mMyList(&lst) {}
   ...
   class iterator : public AKlist<T>::const_iterator {
      friend class AKlist;
   protected:
      // chain to base class
      iterator(Node *p, const AKlist<T> & lst) : const_iterator(p, lst)
      {}
   ...
   const AKlist & operator=(const AKlist & rhs);
   AKlist(const AKlist &rhs) {
      init(); *this = rhs;
   }
...
template <typename T>
void AKlist<T>::push_front(const T& x) {
...
template <typename T>
void AKlist<T>::push_back(const T& x) {
...
template <typename T>
const AKlist<T>& AKlist<T>::operator=(const AKlist & rhs) {
   const_iterator iter;
   if (&rhs == this)
      return *this;
   clear();
...
// definition of nested AKlist<T>::Node class ----------------------
template <typename T>
class AKlist<T>::Node {
public:
   Node *prev, *next;
   ...
   Node(const T& d = T(), Node *prv = NULL, Node *nxt = NULL)
